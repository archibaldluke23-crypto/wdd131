...
function displayRecipe(index) {
    ...
    main.innerHTML += `
        <div class="recipe">
            <img src="${recipe.image}" alt="image of meal">
            <div class="tagContainer">
            ${recipe.tags.map(tag => `<h3 class="food-type">${tag}</h3>`).join("")}
            </div>
            <h2>${recipe.name}</h2>
            <span class="rating" role="img" aria-label="Rating: ${recipe.rating} out of 5 stars">
                ${starsHTML}
            </span>
            <p id="description" class="hide">
                ${recipe.description}
            </p>
        </div>
    `;
...
function filterRecipes(search){
    ...
    {
        displayRecipe(recipes.indexOf(results[i]));
    }
}
